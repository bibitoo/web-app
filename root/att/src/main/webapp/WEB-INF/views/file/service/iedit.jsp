...
<head>
	...
	var deletedIds = ${empty param.deletedIds?"[]":param.deletedIds};
	var modified = ${empty param['modified']?'false':param['modified']};
	...
	function getResult(param){
		...
		retval.deletedIds = deletedIds;
		retval.modified = modified;
		var rows = new Array();
		$("tbody.files>tr").each(function(index){
			var attachment = new Object();
			attachment.id = $(".id-checkbox",this).val();
			attachment.title = $("input[name='title']",this).val();
			attachment.remark = $("input[name='remark']",this).val();
			rows.push(attachment);
		});
		retval.rows = rows;
		console.log("---call function getResult:"+JSON.stringify(retval));
	...
	$(document).ready(function() {
		...
		$("#listTable input[type=text]").on("keyup",function(){
			modified=true;
		});
				if(titleSuggest){
			    		...
			    		$("#list input.title-input").each(function(index){
			    			var $this = $(this);
							$(this).typeahead({source: titleSuggest, items:5});
						});
				...
				if(remarkSuggest){
			    		$("#list input.remark-input").each(function(index){
			    			var $this = $(this);
							$(this).typeahead({source: remarkSuggest, items:5});
						});
...
<fieldset>
	...
	<form id="fileupload" action="//jquery-file-upload.appspot.com/" method="POST" enctype="multipart/form-data">
        ...
        <div class="row fileupload-buttonbar">
		    ...
		    <div id="list">
				<input type="hidden" name="saveOther" value="false"/>
...
{% for (var i=0, file; file=o.files[i]; i++) { %}
    <tr class="template-download fade">
		<td><label class="checkbox"><input type="checkbox" class="lk-check id-checkbox" oid="{%=file.id%}" value="{%=file.id%}"></label></td>
			<td><div class="input-append">
				<input type="text" name="title" oid="{%=file.id%}" data-provide="typeahead" class="title-input input-small" value="{%=file.title%}"/>
			...
			<td><div class="input-append">
				<input type="text" name="remark" oid="{%=file.id%}" data-provide="typeahead" class="remark-input input-small" value="{%=file.remark%}"/>
				</td>
