...
<html>
<head>
	<title><fmt:message key="entity.employee" bundle="${org}"/></title>
	<script>
	...
	var setting = {
			...
			callback:{onClick:employeeTreeOnClick, onAsyncSuccess: employeeOnAsyncSuccess}

		};
	var firstAsyncSuccessFlag = 0;
	function employeeOnAsyncSuccess(event, treeId, treeNode, msg) {
		var tree_employee = $.fn.zTree.getZTreeObj("tree_employee");

		if(idPaths[0] == ""){
			if (firstAsyncSuccessFlag == 0) {
				firstAsyncSuccessFlag = 1;
				var nodes = tree_employee.getNodes();
				tree_employee.expandNode(nodes[0], true);
			}
		}
		lk_expandAjaxTreeNode(treeNode,idPaths,tree_employee);
	};
	


	function employeeTreeOnClick(event, treeId, treeNode) {
	...
	$(document).ready(function() {
		...
		ztree = $.fn.zTree.init($("#tree_employee"), setting);
		 $("#menu-collapse").togglepanels();
			...
			useSelectedPanel:true,//isCheckSelect:false,
				...
				columns: [{name:"id",label:"ID"},
							{name:"name",label:"<fmt:message key="entity.employee.name" bundle="${org}"/>"}
						,
						{name:"virtual",label:"<fmt:message key="entity.employee.virtual" bundle="${org}"/>"}
						,
						{name:"createTime",label:"<fmt:message key="entity.employee.createTime" bundle="${org}"/>"}
						,
						{name:"lastModifyTime",label:"<fmt:message key="entity.employee.lastModifyTime" bundle="${org}"/>"}
	...
	function renderField(row,column,settings){
	  		if(column == 'name'){
			if(row[column]){
				return '<a href="${ctx}/org/employee/view/'+row['id']+'">'+row[column]+'</a>';
			}else{
				return " ";
			}
		}
		...
		bootbox.confirm("<fmt:message key="common.form.delete.confirm"/>", function(result) {                
			  if (result) {  		
				if(paginationList){
					var ids = $.paginationList("#list").getSelectedIds();
					if(ids){
						$.get("${ctx}/org/employee/disableAll",{ids:ids},function (data, textStatus){
...
<body>
<fieldset>
<c:set var="menuKey" value="Employee"/>
	...
	<div class="span3">
		<div id="menu-collapse">
			<div>
	             ...
	             <div><ul id="tree_employee" class="ztree"></ul></div>
	         </div>
	     </div>
	...
	<div class="span9">
		...
		<div id="list">
			<div class="conditionPanel">
				...
				<button type="button" class="btn btn-primary" id="add_btn" onclick="window.location.href='${ctx}/org/employee/create?parentId='+getSelectedNodeId();"><fmt:message key="common.form.create"/></button>
