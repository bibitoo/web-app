...
<html>
<head>
	<title><fmt:message key="entity.department" bundle="${org}"/></title>
	<script>
	...
	var setting = {
			...
			callback:{onClick:departmentTreeOnClick, onAsyncSuccess: departmentOnAsyncSuccess}

		};
	var firstAsyncSuccessFlag = 0;
	function departmentOnAsyncSuccess(event, treeId, treeNode, msg) {
		var tree_department = $.fn.zTree.getZTreeObj("tree_department");
		
		if(idPaths[0] == ""){
			if (firstAsyncSuccessFlag == 0) {
				firstAsyncSuccessFlag = 1;
				var nodes = tree_department.getNodes();
				tree_department.expandNode(nodes[0], true);
			}
		}
		lk_expandAjaxTreeNode(treeNode,idPaths,tree_department);
	};
	...
	$(document).ready(function() {
		...
		ztree = $.fn.zTree.init($("#tree_department"), setting);
		 $("#menu-collapse").togglepanels();
			...
			useSelectedPanel:true,//isCheckSelect:false,
				...
				columns: [{name:"id",label:"ID"},
							{name:"name",label:"<fmt:message key="entity.department.name" bundle="${org}"/>"}
						,
						{name:"virtual",label:"<fmt:message key="entity.department.virtual" bundle="${org}"/>"}
						,
						{name:"createTime",label:"<fmt:message key="entity.department.createTime" bundle="${org}"/>"}
						,
						{name:"lastModifyTime",label:"<fmt:message key="entity.department.lastModifyTime" bundle="${org}"/>"}
	...
	function renderField(row,column,settings){
	  		if(column == 'name'){
			if(row[column]){
				return '<span  class="list-subject"><a href="${ctx}/org/department/view/'+row['id']+'">'+row[column]+'</a></span>';
			}else{
				return " ";
			}
		}
		...
		bootbox.confirm("<fmt:message key="common.form.delete.confirm"/>", function(result) {                
			  if (result) {  		
				if(paginationList){
					var ids = $.paginationList("#list").getSelectedIds();
					if(ids){
						$.get("${ctx}/org/department/disableAll",{ids:ids},function (data, textStatus){
...
<body>
<fieldset>
<c:set var="menuKey" value="Department"/>
...
<div class="row">
	<div class="span3">
		...
		<div>
             ...
             <div><ul id="tree_department" class="ztree"></ul></div>
         </div>
     </div>
	...
	<div class="span9">
		...
		<div id="list">
			<div class="conditionPanel">
				...
				<button type="button" class="btn btn-primary" id="add_btn" onclick="window.location.href='${ctx}/org/department/create?parentId='+getSelectedNodeId();"><fmt:message key="common.form.create"/></button>
