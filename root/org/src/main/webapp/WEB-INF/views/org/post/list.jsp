...
<html>
<head>
	<title><fmt:message key="entity.post" bundle="${org}"/></title>
	<script>
	...
	var setting = {
			...
			callback:{onClick:postTreeOnClick, onAsyncSuccess: postOnAsyncSuccess}

		};
	var firstAsyncSuccessFlag = 0;
	function postOnAsyncSuccess(event, treeId, treeNode, msg) {
		var tree_post = $.fn.zTree.getZTreeObj("tree_post");

		if(idPaths[0] == ""){
			if (firstAsyncSuccessFlag == 0) {
				firstAsyncSuccessFlag = 1;
				var nodes = tree_post.getNodes();
				tree_post.expandNode(nodes[0], true);
			}
		}


		lk_expandAjaxTreeNode(treeNode,idPaths,tree_post);
	};
	


	function postTreeOnClick(event, treeId, treeNode) {
		$.paginationList("#list").addSelectedCondition("search_EQ_parent.id",'<fmt:message key="entity.post" bundle="${org}"/>:'+treeNode.name,treeNode.id);
		}
	var paginationList;
	var ztree ;
	$(document).ready(function() {

		ztree = $.fn.zTree.init($("#tree_post"), setting);
		 ...
		 $("#menu-collapse").togglepanels();
			...
			useSelectedPanel:true,//isCheckSelect:false,
				...
				columns: [{name:"id",label:"ID"},
							{name:"name",label:"<fmt:message key="entity.post.name" bundle="${org}"/>"}
						,
						{name:"virtual",label:"<fmt:message key="entity.post.virtual" bundle="${org}"/>"}
						,
						{name:"createTime",label:"<fmt:message key="entity.post.createTime" bundle="${org}"/>"}
						,
						{name:"lastModifyTime",label:"<fmt:message key="entity.post.lastModifyTime" bundle="${org}"/>"}
	...
	function renderField(row,column,settings){
	  		if(column == 'name'){
			if(row[column]){
				return '<a href="${ctx}/org/post/view/'+row['id']+'">'+row[column]+'</a>';
			}else{
				return " ";
			}
		}
		...
		bootbox.confirm("<fmt:message key="common.form.delete.confirm"/>", function(result) {                
			  if (result) {  		
				if(paginationList){
					...
					if(ids){
						$.get("${ctx}/org/post/disableAll",{ids:ids},function (data, textStatus){
...
<div class="row">
	<div class="span3">
		...
		<div>
             <h3><a href="#">&nbsp;<fmt:message key="entity.post" bundle="${org}"/></a></h3>
             <div><ul id="tree_post" class="ztree"></ul></div>
         </div>
     </div>
	...
	<div class="span9">
		...
		<div id="list">
			<div class="conditionPanel">
				...
				<button type="button" class="btn btn-primary" id="add_btn" onclick="window.location.href='${ctx}/org/post/create?parentId='+getSelectedNodeId();"><fmt:message key="common.form.create"/></button>
