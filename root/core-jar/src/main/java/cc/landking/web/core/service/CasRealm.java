    ...
    */
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
        ...
        try {
    		...
    		User user = userService.findUserByLoginName(userId);
    		if ((user != null)  && (!user.isForbidden())) {
    			...
    			suser.getRoles().addAll(split(defaultRoles));
    			byte[] salt = Encodes.decodeHex(user.getSalt());
    			userService.resetUserRole(suser);
    		
    			return new SimpleAuthenticationInfo(suser, ticket, getName());
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		if (object != null && object instanceof LandkingUser) {
