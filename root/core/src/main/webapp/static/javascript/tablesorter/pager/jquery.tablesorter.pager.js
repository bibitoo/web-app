	...
	$.extend({ tablesorterPager: new function() {
		...
		pagerArrows = function(c, disable) {
			...
			if ( c.updateArrows ) {
				...
				var key = ( dis || (c.page === tp - 1) || (tp === 0)) ? a : r ;
				c.$container.find(c.cssNext + ',' + c.cssLast)[ key ](d);
			}
		},
		...
		updatePageDisplay = function(table, c, flag) {
			...
			if ( Math.min( c.totalPages, c.filteredPages ) >= 0 ) {
				...
				if (out.length) {
					...
					if ( c.$goto.length && out[0].tagName === 'SELECT') {
						t = '';
		...
		renderAjax = function(data, table, c, xhr, exception){
			...
			if ( typeof(c.ajaxProcessing) === "function" ) {
				...
				var resultRowSize = result[t ? 1 : 0] || 0;
				if ( exception ) {
		...
		$this.construct = function(settings) {
			return this.each(function() {
				...
				$t
					...
					.bind('refresh.pager', function(e,v){
						moveToPage(table, c, false);
						updatePageDisplay(table, c, false);
						fixHeight(table, c);
					})
